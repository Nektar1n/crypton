...
export default {
  ...
  data(){
    ...
    add(){
      ...
      setInterval(async ()=>{
        const f=await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${newTicker.valute}&tsyms=USD&api_key=b2a0c50f320ffb15bb2144476856e07ddfde53ea81f697fe8792730fff8f5fb2`)
        const data=await f.json();

        this.tickers.find(tick=>tick.valute===newTicker.valute).price=data.USD>1?data.USD.toFixed(4):data.USD.toPrecision(4)
        if (this.sel?.valute===newTicker.valute){
